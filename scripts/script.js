...
function bindFunctions() {
  ...
  operand.forEach((operand) =>
    operand.addEventListener("click", operandClicked)
  );
  ...
  if (result.value >= 1) {
    if (!isOperand(result.value.charAt(result.value.length - 1))) {
      ...
      if (result.value.length > 1) {
        result.value =
          result.value.substring(0, result.value.length - 1) + this.innerText;
        operatorStack.pop();
      }
    } else {
      result.value += this.innerText;
...
function checkPriority(operator) {
  while (
    ...
    priorityTable[onTop(operandStack)] > priorityTable[operator]
  ) {
    let operand1 = parseFloat(operandStack.pop());
    let operand2 = parseFloat(operandStack.pop());
    let operator = operatorStack.pop();
